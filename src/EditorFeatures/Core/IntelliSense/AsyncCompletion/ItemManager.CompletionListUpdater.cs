...
namespace Microsoft.CodeAnalysis.Editor.Implementation.IntelliSense.AsyncCompletion
{
    internal partial class ItemManager
    {
        ...
        private sealed class CompletionListUpdater
        {
            ...
            private static readonly ObjectPool<List<RoslynCompletionItem>> s_listOfRoslynItemPool = new(factory: () => new(), size: 1);
            private static readonly ObjectPool<List<(RoslynCompletionItem RoslynCompletionItem, PatternMatch? PatternMatch)>> s_listOfRoslynMatchResultPool = new(factory: () => new(), size: 1);
            ...
            private ItemSelection? HandleNormalFiltering(IReadOnlyList<MatchResult<VSCompletionItem>> items)
            {
                ...
                // Use a dedicated pool to minimize potentially repeated large allocations,
                // since the completion list could be long with import completion enabled.
                var roslynMatches = s_listOfRoslynMatchResultPool.Allocate();
                var chosenItems = s_listOfRoslynItemPool.Allocate();
                try
                {
                    ...
                    roslynMatches.AddRange(items.Where(r => r.MatchedFilterText).Select(t => (t.RoslynCompletionItem, t.PatternMatch)));

                    _filterMethod(roslynMatches, _filterText, chosenItems);
                    ...
                    else
                    {
                        ...
                        if (_filterText.Length > 0)
                        {
                            ...
                            if (roslynMatches.Count(r => !r.RoslynCompletionItem.IsPreferredItem()) == 1)
                ...
                finally
                {
                    ...
                    roslynMatches.Clear();
                    chosenItems.Clear();
                    s_listOfRoslynMatchResultPool.Free(roslynMatches);
                    s_listOfRoslynItemPool.Free(chosenItems);
                }
            }
